...
function GameContent() {
  ...
  const [userSecret, setUserSecret] = useState(null); // Temp storage for modal
  ...
  const [showEntryModal, setShowEntryModal] = useState(false); // Login/Signup
  const [showKeyModal, setShowKeyModal] = useState(false); // New Account Keys
  const [keysConfirmed, setKeysConfirmed] = useState(false); // Checkbox
  
  const [showDashboard, setShowDashboard] = useState(false); // VAULT (Funds)
  ...
  useEffect(() => {
    const storedPub = localStorage.getItem('sob_pub_key');
    if (storedPub) {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      if(data.success) {
        ...
        localStorage.setItem('sob_pub_key', data.publicKey);
  ...
  const handleLogout = () => {
    localStorage.removeItem('sob_pub_key');
    setUserAddress(null);
    setVaultBalance(0);
    setStreakHistory([]);
    setActivePicks([]);
  ...
  return (
    <div className="h-screen w-screen bg-[#020205] text-[#F0F0F0] flex flex-col overflow-hidden relative font-sans">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -50, opacity: 0 }} className="absolute top-16 left-0 right-0 bg-black/95 backdrop-blur-xl border-b border-white/10 z-[90] p-6 lg:hidden flex flex-col gap-4">
            <button className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-white py-3 border-b border-white/5"><Coins className="w-4 h-4 text-cyan-400" /> BUY $SOB</button>
            <button onClick={() => {if(userAddress) setShowMyBets(true); else setShowEntryModal(true); setIsMenuOpen(false);}} className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-white py-3 border-b border-white/5"><History className="w-4 h-4 text-cyan-400" /> MY BETS</button>
            <button onClick={() => {if(userAddress) setShowStakeSelect(true); else setShowEntryModal(true); setIsMenuOpen(false);}} className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-white py-3 border-b border-white/5"><PlusCircle className="w-4 h-4 text-cyan-400" /> START NEW STREAK</button>
            <button onClick={() => {setShowLiveFeed(true); setIsMenuOpen(false);}} className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-white py-3"><Activity className="w-4 h-4 text-red-500" /> LIVE FEED</button>
            {userAddress && (
              <>
                <button onClick={() => {setShowDashboard(true); setIsMenuOpen(false);}} className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-blue-400 py-3 border-t border-white/10"><User className="w-4 h-4" /> VAULT</button>
                <button onClick={handleLogout} className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-red-500 py-3"><LogOut className="w-4 h-4" /> LOGOUT</button>
              </>
            )}
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showDashboard && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative bg-[#0a0a10] border border-white/10 p-6 rounded-[2rem] w-full max-w-lg shadow-2xl">
              ...
              <div className="space-y-6">
                  <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                    <span className="text-[10px] font-black text-neutral-500 uppercase tracking-widest block mb-2">Deposit Address</span>
                    <div className="flex items-center gap-2 bg-black/50 p-3 rounded-lg border border-white/5">
                      <span className="text-xs font-mono text-cyan-400 break-all">{userAddress}</span>
                      <button onClick={() => handleCopy(userAddress)}><Copy className="w-4 h-4 text-white" /></button>
                    </div>
                    <p className="text-[9px] text-neutral-500 mt-2 text-center">Send SOL here. Funds stay in your custody until you bet.</p>
                  </div>
                  ...
                  <button onClick={handleCheckDeposit} disabled={isLoading} className="w-full py-3 rounded-xl bg-blue-600 text-white font-black text-[10px] uppercase hover:bg-blue-500 flex items-center justify-center gap-2">
                      {isLoading ? <Activity className="w-4 h-4 animate-spin" /> : <ArrowDownCircle className="w-4 h-4" />} Check Deposit
                  </button>

                  <div className="pt-4 border-t border-white/10 space-y-3">
                      <span className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Withdraw</span>
                      <input type="text" placeholder="Destination Address" value={withdrawDest} onChange={(e) => setWithdrawDest(e.target.value)} className="w-full bg-[#0a0a10] border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-mono" />
                      <div className="flex gap-2">
                          <input type="number" placeholder="Amount" value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} className="flex-1 bg-[#0a0a10] border border-white/10 rounded-xl px-4 py-3 text-white text-sm font-bold" />
                          <button onClick={handleMaxWithdraw} className="bg-white/10 px-3 rounded-xl text-[9px] font-bold text-white">MAX</button>
                      </div>
                      <button onClick={handleWithdraw} disabled={isLoading} className="w-full py-3 rounded-xl bg-white/10 text-white font-black text-[10px] uppercase hover:bg-white/20">Withdraw SOL</button>
                  </div>
                  
                  <button onClick={handleLogout} className="w-full mt-2 text-[9px] font-black text-red-500 uppercase hover:text-red-400">Logout</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {showMyBets && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative bg-[#0a0a10] border border-white/10 p-6 rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
              ...
              <div className="overflow-y-auto pr-2 flex-1 space-y-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  {streakHistory.length === 0 && <p className="text-[10px] text-neutral-600 text-center py-4">No active streaks.</p>}
                      ...
                      <div key={streak.id} className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                          <div className="flex justify-between items-center mb-3">
                              ...
                              <div className="text-right">
                                  ...
                                  <span className="text-[8px] font-black text-green-500 uppercase">WIN: {streak.potentialPayout} SOL</span>
                              </div>
                          </div>
                          <div className="flex justify-between text-[8px] font-bold text-neutral-500 uppercase bg-black/20 p-2 rounded mb-2">
                              <span>Status</span>
                              <span className={isEventStarted ? 'text-green-400' : 'text-yellow-500'}>{isEventStarted ? 'LIVE' : 'PENDING'}</span>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                              {streak.picks.slice(0,4).map((p, i) => (
                                  <div key={i} className="text-[7px] text-neutral-400 flex justify-between">
                                      <span>{p.outcome}</span>
      ...
      {/* 3. NEW ACCOUNT KEY MODAL */}
      <AnimatePresence>
        {showKeyModal && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative bg-[#0a0a10] border border-red-500/50 p-8 rounded-[2rem] w-full max-w-lg shadow-2xl">
              <div className="text-center mb-6">
                ...
                <p className="text-xs text-neutral-400">You will NOT see these again.</p>
              </div>
              <div className="space-y-4">
                  <div className="bg-black/50 p-4 rounded-xl border border-white/10">
                      ...
                      <div className="flex justify-between"><span className="text-xs text-white font-mono break-all">{userAddress}</span><button onClick={() => handleCopy(userAddress)}><Copy className="w-4 h-4" /></button></div>
                  </div>
                  <div className="bg-red-900/10 p-4 rounded-xl border border-red-500/30">
                      <span className="text-[9px] text-red-400 uppercase block mb-1">Private Key</span>
                      <div className="flex justify-between"><span className="text-xs text-white font-mono break-all">{userSecret}</span><button onClick={() => handleCopy(userSecret)}><Copy className="w-4 h-4" /></button></div>
                  </div>
              </div>
              <div className="mt-6 flex items-center justify-center gap-3">
                  ...
                  <span className="text-[10px] text-neutral-400 font-bold uppercase cursor-pointer" onClick={() => setKeysConfirmed(!keysConfirmed)}>I saved my key</span>
              </div>
              <button onClick={handleConfirmKeys} disabled={!keysConfirmed} className="mt-6 w-full py-4 rounded-xl bg-white text-black font-black text-[10px] uppercase hover:bg-gray-200 disabled:opacity-50">Continue</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
        ...
        {showEntryModal && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative bg-[#0a0a10] border border-white/10 p-10 rounded-[3rem] w-full max-w-xl shadow-2xl">
              <div className="text-center mb-10"><ShieldCheck className="w-12 h-12 text-blue-500 mx-auto mb-4" /><h3 className="text-3xl font-black italic uppercase text-white mb-2">Player Access</h3></div>
              <div onClick={handleCreateAccount} className="p-6 rounded-2xl bg-black border border-blue-500/30 hover:border-blue-500 cursor-pointer text-center group"><span className="text-[8px] font-black text-blue-500 block mb-1">NEW PLAYER</span><p className="text-xs font-black text-white italic uppercase">Create Account</p></div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative bg-[#0a0a10] border border-white/10 p-10 rounded-[3rem] w-full max-w-xl shadow-2xl text-center">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                 {[0.05, 0.5, 1].map((amt) => (<button key={amt} onClick={() => handleStartStreak(amt)} className="p-6 rounded-2xl bg-white/5 border border-white/20 hover:bg-white hover:text-black transition-all group"><span className="text-2xl font-black block">{amt} SOL</span><span className="text-[9px] font-bold uppercase opacity-50 group-hover:opacity-100">Lock Stake</span></button>))}
              </div>
              <div className="flex gap-2 mb-6">
                  <input type="number" placeholder="Custom SOL (0.05-500)" value={customStake} onChange={(e) => setCustomStake(e.target.value)} className="flex-1 bg-[#0a0a10] border border-white/10 rounded-xl px-4 text-white font-bold h-12" />
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative bg-[#0a0a10] border border-white/10 p-8 rounded-[2rem] w-full max-w-lg shadow-2xl h-[500px] flex flex-col">
              <div className="flex justify-between items-center mb-6 flex-none border-b border-white/5 pb-4">
                <div className="flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_8px_#ef4444]" /><h3 className="text-xl font-black italic uppercase text-white tracking-tight">Arena Feed</h3></div>
                ...
                {[...Array(15)].map((_, i) => (
                  <div key={i} className="flex justify-between items-center p-4 rounded-xl bg-gradient-to-r from-white/[0.02] to-transparent border border-white/5 hover:border-cyan-500/20 transition-colors">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${Math.random() > 0.5 ? 'bg-green-500/10 text-green-400' : 'bg-blue-500/10 text-blue-400'}`}>{Math.random() > 0.5 ? <TrendingUp className="w-3 h-3" /> : <ArrowDownCircle className="w-3 h-3" />}</div>
                        <div className="flex flex-col"><span className="text-[10px] font-bold text-white">User_{Math.floor(Math.random()*9999)}</span><span className="text-[8px] font-black text-neutral-500 uppercase tracking-widest">{Math.random() > 0.5 ? 'WON STREAK' : 'VAULT DEPOSIT'}</span></div>
                    </div>
                    <div className="text-right"><span className="text-xs font-black text-cyan-400 block">{(Math.random() * 5).toFixed(2)} SOL</span><span className="text-[8px] font-bold text-neutral-600">Just now</span></div>
                  </div>
                ))}
              </div>
            </motion.div>
