...
function GameContent() {
  ...
  // ACCOUNT STATE (REPLACES WALLET ADAPTER)
  ...
  const [withdrawDestination, setWithdrawDestination] = useState(''); // NEW

  // UI State
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [showEntryModal, setShowEntryModal] = useState(false); // Used for Login/Create
  const [showNewAccountModal, setShowNewAccountModal] = useState(false); // Shows Keys
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setKeysConfirmed(false); // Reset confirmation
        
        // Save public key ONLY to local storage
  ...
  const handleConfirmKeys = () => {
    ...
    // User confirms they saved their keys
    setShowNewAccountModal(false);
    setUserSecret(null); // Clear secret from state for safety
  ...
  const handleRefreshBalance = async () => {
    ...
    setIsLoading(true);
    try {
      // Re-use streak endpoint which returns on-chain balance
      const res = await fetch(`/api/streak?userAddress=${userAddress}`);
      const data = await res.json();
      if (data.realBalance !== undefined) {
        setVaultBalance(data.realBalance);
        alert(`Balance Synced: ${data.realBalance.toFixed(3)} SOL`);
      }
    } catch (e) {
  ...
  const handleWithdraw = async () => {
    ...
    if (!withdrawDestination) return alert("Enter destination address");
  ...
  const handleMaxWithdraw = () => {
      // Leave tiny amount for gas if needed, or just set max
      setWithdrawAmount((vaultBalance - 0.002).toFixed(4));
  };
              ...
              <div className="overflow-y-auto pr-2 flex-1 space-y-6" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  ...
                  <div className="border-t border-white/10 pt-6">
                      ...
                      <div className="flex flex-col gap-2 pt-4 border-t border-white/5">
                          ...
                          <input type="text" placeholder="Destination Address" value={withdrawDestination} onChange={(e) => setWithdrawDestination(e.target.value)} className="w-full bg-[#0a0a10] border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-mono focus:outline-none focus:border-white transition-colors" />
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative bg-[#0a0a10] border border-white/10 p-10 rounded-[3rem] w-full max-w-xl shadow-2xl">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                {/* Future: Add 'Restore Account' here */}
              </div>
            </motion.div>
          </div>
        )}
